...
public class LocaleUtil {
    ...
    static final String tag = "currentLanguage";
    /**
     * 获取用户设置的Locale
     * @return Locale
     */
    public static Locale getUserLocale() {
        int currentLanguage = SpUtil.getInstance().getInt(tag, 0);
    ...
    public static void changeAppLanguage(Context context) {
        ...
        int currentLanguage = SpUtil.getInstance().getInt(tag, -1);
        Locale myLocale;
    ...
    public static void changeAppLanguage(Context context, int currentLanguage) {
        if (context == null) return;
        Context appContext = context.getApplicationContext();
        SpUtil.getInstance().save(tag, currentLanguage);
        Locale myLocale = Locale.SIMPLIFIED_CHINESE;
        // 0 简体中文 1 繁体中文 2 English
        switch (currentLanguage) {
            case 0:
        ...
        Toast.makeText(appContext, "设置成功", Toast.LENGTH_SHORT).show();
